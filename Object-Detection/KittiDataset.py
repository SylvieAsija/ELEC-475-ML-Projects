...
class KittiDataset(Dataset):
    def __init__(self, dir, training=True, transform=None):
        ...
        if self.training == False:
            self.mode = 'test'
        self.img_dir = os.path.join(dir, self.mode, 'image').replace("\\", "/")
        self.label_dir = os.path.join(dir, self.mode, 'label').replace("\\", "/")
    ...
    def __getitem__(self, idx):
        ...
        #     label = [lsplit[0], int(self.class_label[lsplit[0]]), float(lsplit[4]), float(lsplit[5]), float(lsplit[6]),
        #              float(lsplit[7])]
        #     labels += [label]
        # return image, labels
        filename = os.path.splitext(self.img_files[idx])[0]
        ...
        # label_path = os.path.join(self.label_dir, filename + '.txt').replace("\\", "/")
    ...
    # class_label = {'DontCare': 0, 'Misc': 1, 'Car': 2, 'Truck': 3, 'Van': 4, 'Tram': 5, 'Cyclist': 6, 'Pedestrian': 7, 'Person_sitting': 8}
