...
def main():
    ...
    for item in enumerate(dataset):
        ...
        for k in range(len(boxes)):
            ...
            if save_ROIs == True:
                cv2.imwrite(os.path.join('./data/Kitti8_ROIs/train/image', filename).replace("\\", "/"), ROIs[k])
            name_class = 0
            name = 'NoCar'
    ...
    #
    if save_ROIs == True:
        with open(os.path.join('./data/Kitti8_ROIs/train/label', label_file).replace("\\", "/"), 'w') as f:
